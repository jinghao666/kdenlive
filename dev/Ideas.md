...
| **Modifier Keys** | 🤓 – More consistant to other Software | We already have tons of actions performed with combinations of Shift/Ctrl/Alt/Double click, sometimes not very coherent. Make it more consistent with clear rules what they do. [What modifier keys are][WMKA]. Here lists of Premiere shortcuts: [good to see what shift does](http://ultrashortcuts.com/premiere), [standard shorcut](https://helpx.adobe.com/premiere-pro/using/keyboard-shortcuts.html). [Pre-Defined key-combination in Windows](https://docs.microsoft.com/en-us/windows-hardware/customize/enterprise/predefined-key-combinations). Here some user requests #332, #674, #337. |
| **General usability improvements** | 🎥 – Kdenlive provides a consistent and practical UX | Redesign clip popups (e.g. other location) to avoid that they overlap controls (#592), make it easier to resize the timeline (#593), use timeline scrubbing/zooming for faster navigation in the timeline, see [T10897][T10897], highlight the active panel (has shortcut focus) |
...
[WMKA]: https://en.wikipedia.org/wiki/Modifier_key
